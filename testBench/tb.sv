...
module top(
    ...
    input logic fastClk,
    input logic rst,
    input [12:0] serialData,
    input logic write_enable,
    output logic [2:0] dout
);

    // Instantiate the queue module
    wire [12:0] qout [0:299];
    
    queue #(
        .QUEUE_SIZE(300),
        .DATA_WIDTH(13)
    ) q (
        .clk(slowClk), // Use slowClk for the queue
        .rst(rst),
        .write_enable(write_enable), // Connect write enable signal
        .serialData(serialData),
        .data(qout)
    );

    // Instantiate the ECGPreprocess module
    ...
    ECGPreprocess #(
        ...
        .clk(fastClk), // Use fastClk for preprocessing
        .rst(rst),
        .start(1'b1),
        .ecg_signal(qout),
 ...
 module DFF(
    ...
    always @(posedge clk) begin
        if(reset)begin
            q <= 0; // Use non-blocking assignment
        end 
        else begin
            q <= d;
        end
...
module queue #(
    ...
    input  logic clk,
    input  logic rst,
    input  logic write_enable, // Add write enable signal
...
module ECGPreprocess #(
            ...
            case (state)
                ...
                FIND_PEAKS: begin
                    ...
                    if (current_index < SIGNAL_LENGTH - 1) begin
                        if (current_index > 0 &&
                            ecg_signal[current_index] > ecg_signal[current_index - 1] &&
                            ecg_signal[current_index] > ecg_signal[current_index + 1] &&
                            ecg_signal[current_index] >= HEIGHT_TH) begin
                ...
                PROCESS_SIGNAL: begin
                    ...
                    if (peak_count >= 2 && current_index < OUTPUT_LENGTH) begin
                        ...
                        if ((peaks[0] + current_index) <= (peaks[1] + 40) &&
                            ...
                            processed_signal[current_index] <= ecg_signal[peaks[0] + current_index];
                            $display("Processed signal[%0d] = %b", current_index, ecg_signal[peaks[0] + current_index]);
                        end else begin
    ...
    // Print processed signal when done
    always_ff @(posedge clk) begin
        if (done) begin
            $display("Processed Signal:");
            for (i = 0; i < OUTPUT_LENGTH; i = i + 1) begin
                $display("%d: %b", i, processed_signal[i]);
            end
        end
    end
