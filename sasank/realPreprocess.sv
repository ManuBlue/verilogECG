module ECGPreprocess #(
    parameter SIGNAL_LENGTH = 1000,        // Total number of signal samples
    ...
    parameter DISTANCE    = 120             // Minimum distance between peaks
)(
    input  logic [DATA_WIDTH-1:0] ecg_signal [0:SIGNAL_LENGTH-1],
    output logic [DATA_WIDTH-1:0] processed_signal [0:SIGNAL_LENGTH-1]
);
  ...
  always_comb begin
    ...
    if (filtered_count >= 2) begin
      ...
      // Copy the segment from the original signal.
      for (i = seg_start; i <= seg_end; i = i + 1) begin
        processed_signal[i] = ecg_signal[i];
      end
      ...
      for (i = seg_end; i >= seg_start; i = i - 1) begin
        if (processed_signal[i] >= ahh)
          break;  // exit the loop when a value >= ahh is found
        else
          processed_signal[i] = '0;
      end
    end
