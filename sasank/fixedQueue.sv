...
module queue #(
    parameter QUEUE_SIZE = 187,  // Parameter to define the size of the queue
    parameter DATA_WIDTH = 13     // Parameter to define the width of the data
)(
    input logic clk,
    input logic rst,
    input logic [DATA_WIDTH-1:0] serialData,
    output reg [DATA_WIDTH-1:0] data [0:QUEUE_SIZE-1]
);

    // Generate shift registers for each bit of the data width
    genvar i;
    generate
...
module ECGPreprocess #(
    ...
    always@(*) begin
        ...
        for (i = 0; i < SIGNAL_LENGTH-1; i = i + 1) begin
            if (i-1>=0 && ecg_signal[i] > ecg_signal[i-1] && 
        ...
        if (peak_count >= 2) begin           
            for (i = 0; i < OUTPUT_LENGTH ; i = i + 1) begin
                if((peaks[0] + i) <= (peaks[1] + 40) &&(peaks[0] + i) < SIGNAL_LENGTH) begin
                    processed_signal[i] = ecg_signal[peaks[0] + i];
                    if (processed_signal[i] > max_value) max_value = processed_signal[i];
                    if(processed_signal[i] < min_value) min_value = processed_signal[i];
                end
            end
